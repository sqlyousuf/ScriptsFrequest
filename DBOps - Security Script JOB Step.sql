...
This is login/user informaiton from linked server POADBP001 so a linked server first has to be created on all instances where this script will be used.
*/
/*
...
Select Account,Production from openquery(POADBP001,
...
Insert into #AccountMember ([AC_Account]) select distinct account from openquery (POADBP001,'select distinct account from msdb.[dbo].[Securityaccounts]') 
...
--'Select Account,Production from msdb.[dbo].[Securityaccounts]') where production in (select name from sys.database_principals where type='R' and name!='PUBLIC')
			...
			from openquery(POADBP001,'select  distinct Account,Production,0 from msdb.[dbo].[Securityaccounts]')
			...
			from openquery(POADBP001,'select distinct Account,Production,1 from msdb.[dbo].[Securityaccounts]')
			 where production In (select name from sys.database_principals where type='R' and name!='PUBLIC') and PRODUCTION !='NO'
			and Account=@account
			except
