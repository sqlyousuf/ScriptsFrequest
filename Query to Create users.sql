...
Insert into #AccountDetails ([AccountName],[AccountAccess])
--select Account,Production from msdb.dbo.[Securityaccounts] with(nolock) 
--where Production in (select name,* from sys.database_principals where type='R' and name!='PUBLIC') 
Select Account,Production from openquery(DTBHSFHOU1277,
'Select Account,Production from enterpriseDBA.[dbo].[Securityaccounts]') where production in (select name from sys.database_principals where type='R' and name!='PUBLIC')
...
Insert into #AccountMember ([AC_Account]) select distinct account from openquery (DTBHSFHOU1277,'select distinct account from enterpriseDBA.[dbo].[Securityaccounts]') 
...
--			select Account,Production from openquery(DTBHSFHOU1277,
--'Select Account,Production from msdb.[dbo].[Securityaccounts]') where production in (select name from sys.database_principals where type='R' and name!='PUBLIC')

			SELECT AccName,AccAccess,0 FROM @AccountAccessdblevel where AccName =@account
			Except
			select distinct Account,Production,0 
		
			from openquery(DTBHSFHOU1277,'select  distinct Account,Production,0 from enterpriseDBA.[dbo].[Securityaccounts]')
				 where production In (select name from sys.database_principals where type='R' and name!='PUBLIC') and PRODUCTION !='NO'
			and Account=@account
			)
			...
			(
			select distinct Account,Production,1 
			from openquery(DTBHSFHOU1277,'select distinct Account,Production,1 from enterpriseDBA.[dbo].[Securityaccounts]')
			 where production In (select name from sys.database_principals where type='R' and name!='PUBLIC') and PRODUCTION !='NO'
			and Account=@account
			except
...
select distinct Account,Production,0 
			...
			from openquery(DTBHSFHOU1277,'select  distinct Account,Production,0 from enterpriseDBA.[dbo].[Securityaccounts]')
				 where production In (select name from sys.server_principals where type='R' and name!='PUBLIC') 
